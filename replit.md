...
**Режим Мульти AI** включён по умолчанию 🎯. Perplexity Large (бесплатно, 0₽/1K) анализирует задачу и делегирует выполнение 1–3 лучшим моделям. Если Perplexity не выдаёт JSON-решение, fallback на Qwen 3 14B (0.045₽/1K).
...
Бюджет на один запрос: **2₽** (`BUDGET_RUB` в `public/app.js`). Vision-модели с картинками стоят дороже (base64 → много токенов), поэтому лимит в настройках VseGPT должен быть **не меньше 2₽**:

1. Откройте https://vsegpt.ru/User/SettingsModels
2. Найдите поле «Лимит: 1 запрос не должен стоить больше, чем X рублей»
3. Установите **2–5₽**
4. Сохраните

**Без этого Мульти AI с картинками будет выпадать с ошибкой «Exceeded soft user limit per query»**.

## Архитектура отказоустойчивости

### Мульти AI (runMulti)
- Perplexity Large (0₽) → анализ + делегирование моделям
- Если все делегаты упали: fallback на Perplexity Large (0₽) → Perplexity Small (0₽) → диагностика с ссылкой на настройки VseGPT

### Роутер (runOrchestrator)
- Perplexity Large (0₽) → JSON-решение о direct/delegate/multi
- Если Perplexity не вернул JSON: fallback на Qwen 3 14B (0.045₽/1K)
- Если всё упало: прямой ответ модели-роутера

### Сжатие картинок
- Каскадное сжатие: 640px q=0.55 → 400px q=0.4 → 300px q=0.3
- Максимальный размер после сжатия: ~80 КБ base64 ≈ 20K токенов
- Укладывается в бюджет 2₽ даже для vision-моделей по 0.055₽/1K

### Обработка ошибок VseGPT
- Ошибки `Exceeded soft user limit` → модель помечается недоступной (sessionStorage)
- Повторная попытка на более дешёвой модели или текстовом fallback
- `socket hang up` → retry через 2.5с (rate-limit между запросами)
